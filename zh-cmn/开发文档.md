...
- 我们的Touch配对基于[ 乐鑫 ESP-TOUCH SDK](https://www.espressif.com/zh-hans/products/software/esp-touch/resources) 实现，但在广播过程中做了加密认证，此部分内容需要询问对接销售人员
...
1. 设备进入配置模式: Touch模式
1. 设备等待APP广播 SSID & Password 
1. 设备获取到 SSID & Password 后连上路由器，返回 touch UDP 确认包
1. 设备建立 TCP Server（HTTP Server）
1. 手机发送: （请求配对）
...
备注: {区域}-disp.coolkit.cc，可填cn、as、us、eu，示例: cn-disp.coolkit.cc
...
本质上是设备开放热点，APP连接设备热点后传输正式路由器WIFI信息，完成配网。

与Touch配对1-7步一致，第8步放到最后（先发送POST请求到设备端，再请求添加设备接口），WiFi信息不加密传输。

连上设备的AP后，路由地址为: 10.10.7.1（iOS），安卓可以从底层库中获取到当前路由器IP地址。
