...
- 接口路径:  https://{区域}-api.coolkit.cc:8080/api/user/api/user/tags
- 请求方法:  post
...
| 名称 | 类型 | 允许为空 | 说明 |
| :--- | :--- | :--- | :--- |
| result | number | Y | 1: 账号不存在；2: 用户接受分享；3: 用户不接受分享；4: 已分享 |
| error | number | N | 错误码 |
| apikey | string | N | 用户apikey |
| deviceid | string | N | 设备ID |
| sequence | number | N | 时间戳精确到毫秒，服务器原样返回 |
...
| apikey | string | Y | 用户apikey |
| deviceid | string | Y | 设备ID |
| sequence | number | Y | 时间戳精确到毫秒，服务器原样返回 |
